...
class BabelRuntimeErrorFixture {
    ...
    fun `when running an expression that is index out ofkotli bounds should properly generate error`(){
        ...
        //act
        val ex = assertThrown<RuntimeBabelException> {
            expr.evaluate(immutableMapOf("x1" to 3.0, "x2" to 4.0))
        }
    ...
    @Test fun `when attempting to run multiline expression should properly format`(){
        ...
        //act
        val assertThatEvaluation = assertThatThrownBy {
            expr.evaluate(mapOf("x1" to 0.0))
        }
    ...
    @Test fun `when running with an un-ordered hashmap as globals should eagerly throw`(){
        ...
        //act
        val assertThatEvaluation = assertThatThrownBy {
            expr.evaluate(hashMapOf("x1" to 1.0, "x2" to 2.0))
        }
